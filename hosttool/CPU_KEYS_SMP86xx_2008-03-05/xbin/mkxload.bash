...
Example:
$0 /utils/em8xxx/signed_items 8634_ES4_dev 0006 material.bin material.bin.8634_ES4_dev_0006.bin material.xload
...
EOF
	...
	F=`printf "%d" $SekID`
	if [ $F -lt 7 ]; then
		PK=$XSDK_PUBLIC_KEYS/${REV}_sek${F}_pubkey.pem
		if [ -f "$PK" ]; then
		...
		SEKAES=${XSDK_SEKAES}/${REV}_${H}_sekaes/${REV}_${H}_sekaes${F}.bin
		if [ -f $SEKAES ]; then 
