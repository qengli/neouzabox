...
# Configure network interface and parameters before installing NeouzaBox to disk.
setup_network () {
  ...
  # get NeouzaBox IP address
  val=`grep HOST $f | cut -d'"' -f2`
  host_ip=`$DIALOG --no-cancel --aspect 15 --stdout --backtitle "$title" --title "NeouzaBox IP" --inputbox "\nNeouzaBox needs to be allocated an IP address to be present on your network. Please fill in the following input box or leave it as it for using DHCP autoconfiguration\n" 0 0 "$val"` || exit 1

  # do not get more settings if DHCP
  if [ ! -z $host_ip ]; then
    val=`grep SUBNET $f | cut -d'"' -f2`
    subnet=`$DIALOG --no-cancel --aspect 15 --stdout --backtitle "$title" --title "NeouzaBox Subnet" --inputbox "\nYou may want to connect NeouzaBox to the Internet. Please fill in the following input box with your network Subnet mask or let it blank if you do not want to set a subnet mask for this computer.\n" 0 0 "$val"` || exit 1

    val=`grep GATEWAY $f | cut -d'"' -f2`
    gw_ip=`$DIALOG --no-cancel --aspect 15 --stdout --backtitle "$title" --title "NeouzaBox GateWay" --inputbox "\nYou may want to connect NeouzaBox to the Internet. Please fill in the following input box with your gateway IP address or let it blank if you do not want to set a gateway for this computer.\n" 0 0 "$val"` || exit 1

    val=`grep DNS_SERVER $f | cut -d'"' -f2`
    dns_ip=`$DIALOG --no-cancel --aspect 15 --stdout --backtitle "$title" --title "NeouzaBox DNS Server" --inputbox "\nYou may want to connect NeouzaBox to the Internet. Please fill in the following input box with your DNS Server IP address used for name resolving or let it blank if you do not want to resolve names with this computer.\n" 0 0 "$val"` || exit 1
  fi
...
VERSION=`cat VERSION`
BACKTITLE="NeouzaBox $VERSION installator"

if [ "$UID" != "0" ]; then
  echo ""
  echo "**** You need to be root to install NeouzaBox ****"
  echo ""
  #exit 1
fi

#if [ -z "$SFDISK" -o -z "$GRUB" -o -z "$DIALOG" ]; then
if [ -z "$SFDISK" -o -z "$DIALOG" ]; then
  echo ""
  echo "**** You need to have sfdisk, grub and dialog installed to install NeouzaBox ****"
  echo ""
  exit 1
fi
...
while [ ! -b "$DEV" ]; do
  ...
  if [ -z "$DISKS" ]; then
    ...
    DEV=`$DIALOG --stdout --aspect 15 --backtitle "$BACKTITLE" --title "Installation device" --menu "Where do you want to install NeouzaBox?" 0 0 0 $DISKS` || exit 1
  fi
  if [ ! -b "$DEV" ]; then
...
if [ -d disk ]; then
  ...
  if [ -n "$NFS" ]; then
    ...
    NEOUBOX="$CDROM/NEOUBOX"
  fi
  mkdir -p di/NEOUBOX
  mkdir -p di/boot #added by Leo 2008-1-10
  cp -a "$NEOUBOX/lib" di/NEOUBOX 2>/dev/null
  cp -a "$NEOUBOX/sbin" di/NEOUBOX 2>/dev/null
  cp -a "$NEOUBOX/etc" di/NEOUBOX 2>/dev/null
  cp -a "$NEOUBOX/codecs" di/NEOUBOX 2>/dev/null
  cp -a "$NEOUBOX/var" di/NEOUBOX 2>/dev/null
  cp -a "$NEOUBOX/bin.tar.lzma" di/NEOUBOX/bin.tar.lzma 2>/dev/null
  cp -a "$NEOUBOX/installator/usr.img" di/NEOUBOX/usr.img 2>/dev/null
  cp -a "$NEOUBOX/installator/initrd.installed.gz" di/boot/initrd.gz 2>/dev/null
  
  #cp -a "$NEOUBOX/boot" di/ 2>/dev/null
  #cd di/NEOUBOX/boot
  ##mv vmlinuz isolinux.cfg boot.msg help.msg splash.rle ../../
  #cd ../../../
  #rm -rf di/NEOUBOX/boot
  
  cp -a "$NEOUBOX/boot/vmlinuz" di/boot/ 2>/dev/null
  #cp -a "$NEOUBOX/boot/isolinux.cfg" di/isolinux.cfg 2>/dev/null
  #cp -a "$NEOUBOX/boot/boot.msg" di/boot.msg 2>/dev/null
  #cp -a "$NEOUBOX/boot/help.msg" di/help.msg 2>/dev/null
  #cp -a "$NEOUBOX/boot/splash.rle" di/splash.rle 2>/dev/null
  #leo added 2007-7-19
fi

# Setup network is only available when booting from NeouzaBox.
if [ "$1" = neouzabox ]; then
  $DIALOG --aspect 15 --backtitle "$BACKTITLE" --title "Configure Network ?" --yesno "\nDo you want to configure your network parameters before installing NeouzaBox to disk ?\n" 0 0 && setup_network "di/NEOUBOX"
fi

# Configure TV card and scan for channels.
# (only available when booting from NeouzaBox).
if [ "$1" = neouzabox ]; then
  if grep -q 'Class 0400:.*109e:' /proc/pci ||
    ...
    if [ `echo $?` = 0 ]; then
      $DIALOG --aspect 15 --backtitle "$BACKTITLE" --title "Scan for Analog TV Channels ?" --yesno "\nDo you want to configure your analog tv card and scan for channels before installing NeouzaBox to disk ?\n" 0 0 && setup_tvscan "di/NEOUBOX"
    fi
  fi
fi
...
if [ "$1" = neouzabox ]; then
  ...
  if [ -f /var/dvbcard ]; then
    $DIALOG --aspect 15 --backtitle "$BACKTITLE" --title "Scan for Digital (DVB) TV Channels ?" --yesno "\nDo you want to configure your digital (DVB) tv card and scan for channels before installing NeouzaBox to disk ?\n" 0 0 && setup_dvbscan "di/NEOUBOX"
  fi
fi
...
#added by Leo 2007.11.17 refer to the new version of gbox
[ -f "$NEOUBOX/usr/share/grub-i386-pc.tar.lzma" ] && tar xvaf "$NEOUBOX/usr/share/grub-i386-pc.tar.lzma" -C $grubdir
[ -f "$NEOUBOX/usr/share/grub-i386-pc.tar.gz" ] && tar xzvf "$NEOUBOX/usr/share/grub-i386-pc.tar.gz" -C $grubdir
#lzmacat "$NEOUBOX/usr/share/grub-i386-pc.tar.lzma" | tar xf - -C $grubdir #2007.11.17 Leo add $NEOUBOX 

if [ -f "$NEOUBOX/usr/share/grub-splash.xpm.gz" ]; then #2007.11.17 Leo add $NEOUBOX
  cp -f "$NEOUBOX/usr/share/grub-splash.xpm.gz" $grubdir || exit 1 #2007.11.17 Leo add $NEOUBOX
  disable_splashimage=
else
  disable_splashimage="#"
fi
...
title	NeouzaBox
root	$rootdev_single
kernel	/boot/vmlinuz root=/dev/ram0 rw init=linuxrc boot=$DEVNAME splash=silent vga=0x315 video=vesafb:ywrap,mtrr
initrd  /boot/initrd.gz
boot

title	NeouzaBox (debug)
root	$rootdev_single
kernel	/boot/vmlinuz root=/dev/ram0 rw init=linuxrc boot=$DEVNAME splash=0 vga=0x315 video=vesafb:ywrap,mtrr debugging
initrd  /boot/initrd.gz
boot

title	NeouzaBox (emu)
root	$rootdev_single
kernel	/boot/vmlinuz root=/dev/ram0 rw init=linuxrc boot=$DEVNAME splash=0 vga=0x314 video=vesafb:ywrap,mtrr debugging
initrd  /boot/initrd.gz
boot
EOF
fi
...
'
  ...
  if [ -n "$supported_os_list" ]; then
    ...
    $DIALOG --aspect 15 --backtitle "$BACKTITLE" --title "Bootloader" --defaultno --yesno "\n'$DEV' is now a NeouzaBox partition. To boot from it, you will need to install a bootloader. I can install one for you. If you have any other operating system on your computer, I will also install a multiboot for you. If you do not want me to install a new bootloader, you will need to configure yours alone.\nI have found: $supported_os_list\nDo you want to install me to install the boot loader (GRUB) for you ?\n" 0 0 && MBR=yes
  else
sleep 3
    $DIALOG --aspect 15 --backtitle "$BACKTITLE" --title "Bootloader" --defaultno --yesno "\n'$DEV' is now a NeouzaBox partition. I didn't recognize any other OS on your system, want me to install boot loader on your MBR?\n" 0 0 && MBR=yes
  fi
elif [ $TYPE = REMOVABLE ]; then
  oslist=
  MBR=yes
