...
if [ -d ./iso -a -d ./lirc -a -d ./i18n ]; then
  TMPDIR="."
  GEEXBOX_DIR="."
elif [ -d /usr/share/neoubox-generator ]; then
  TMPDIR="/tmp/neoubox-generator"
  GEEXBOX_DIR="/usr/share/neoubox-generator"
else
  echo ""
...
if [ -e /etc/neoubox-generator.conf ]; then
  . /etc/neoubox-generator.conf;
fi

if [ -e ~/.neoubox-generator ]; then
  . ~/.neoubox-generator.conf;
fi
...
OUTPUT=neoubox-`cat $GEEXBOX_DIR/VERSION`-$LANG.$TARGET_ARCH.iso
...
"$MKISOFS" -quiet -no-pad -V GEEXBOX -volset GEEXBOX \
        -publisher "Neouza Soft" \
        -p "The Neouza Software " \
